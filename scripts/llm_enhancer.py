...
def _build_parking_planner_prompt(question: str, schema_text: str, glossary_text: str) -> str:
    ...
    return (
        ...
        "3. analysis_job 推荐使用以下之一：operational_overview, anomaly_focus, flow_or_occupancy, revenue_focus。\n"
        "4. decision_scope 只能是 executive 或 operations。\n"
        "5. deliverable 对管理层汇报类只能是 web_report 或 daily_brief；非报表类可返回 null。\n"
        "6. time_scope 必须包含 preset, start, end；如果缺时间范围可返回 preset=all 且 start/end=null。\n"
        ...
        "9. implicit_requirements 可用于表达 summary_first、actionable_recommendations 等隐含要求。\n\n"
        "输出 JSON 模板：\n"
        "{"
        "\"domain\":\"parking_ops\","
