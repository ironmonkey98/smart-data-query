...
def _query_data_date_range() -> str:
    ...
    try:
        ...
        if rows and rows[0].get("max_date"):
            ...
            return (
                f"\n\n## 数据库日期范围（重要）\n\n"
                f"数据库中停车经营数据的实际日期范围为 **{min_d} ～ {max_d}**。\n"
                f"- 禁止在 SQL 中使用 `date('now')` 或系统当前日期，因为当前日期已超出数据范围。\n"
                f"- 当用户说"今天/昨天/本周/本月/最近N天"时，以 **{max_d}** 作为"今天"的基准推算日期。\n"
                f"- 如果某个查询返回 0 行，优先考虑是日期范围不在 {min_d}～{max_d} 内，"
                f"直接告知用户并调整到有效范围，不要反复重试相似查询。"
            )
    except Exception:
        pass
    return ""
